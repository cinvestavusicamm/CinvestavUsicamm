...
6 GB VRAM of the Phi-3 / Llama 3.2 model from concurrent requests.
"""

import asyncio

# Importaciones limpias apuntando a la raíz 'src'
from application.validate_process import ValidateProcessUseCase
from application.ingest_rules import IngestRulesUseCase
from application.delete_rules import DeleteRulesUseCase  # <-- NUEVA IMPORTACIÓN
from infrastructure.adapters.ollama_llm import OllamaLLMAdapter
from infrastructure.adapters.qdrant_vector_repo import QdrantVectorRepo
...
def get_delete_rules_use_case() -> DeleteRulesUseCase:
    """Factory that provides a fully-wired DeleteRulesUseCase."""
    return DeleteRulesUseCase(
        vector_repo=vector_repo
    )